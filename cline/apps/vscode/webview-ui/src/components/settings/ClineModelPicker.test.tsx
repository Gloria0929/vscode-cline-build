...
describe("ClineModelPicker", () => {
	beforeEach(() => {
		...
		vi.mocked(useProviderModels).mockReturnValue({
			models: {
				"cline-default": { name: "Coder Default", supportsPromptCache: true },
				"cline-next": {
					name: "Coder Next",
					supportsPromptCache: true,
		...
		vi.mocked(useDynamicProviderSelection).mockReturnValue({
			...
			selectedModelInfo: { name: "Coder Default", supportsPromptCache: true },
			hideUsageCost: false,
		})
	})
	...
	it("commits Cline model selections through provider config so providers.json is updated", async () => {
		...
		fireEvent.click(await screen.findByText("Coder Next"))
