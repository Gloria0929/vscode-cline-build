				...
				<AdvancedSection>
					...
					{!hideUsageCost && hasTiers && (
						<div style={{ marginTop: 8 }}>
							<div style={{ fontWeight: 500, marginBottom: 4 }}>阶梯定价：</div>
							{modelInfo.tiers && (
								<>
									<div>
					...
					{showProviderRouting && onProviderSortingChange && (
						<ProviderRoutingContainer>
							...
							<p
								...
								{!providerSorting &&
									"在各提供商（AWS、Google Vertex 等）之间负载均衡，在考虑正常运行时间的同时优先选择价格更低的提供商"}
								{providerSorting === "price" && "按价格排序，优先选择成本最低的提供商"}
								{providerSorting === "throughput" &&
									"按吞吐量排序，优先选择吞吐量最高的提供商（可能增加成本）"}
								{providerSorting === "latency" && "按响应时间排序，优先选择延迟最低的提供商"}
							</p>
						</ProviderRoutingContainer>
					)}
				</AdvancedSection>
