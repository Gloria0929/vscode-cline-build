		...
		<div className="flex flex-col gap-1">
			...
			{selectedAuthentication === "profile" ? (
				<DebouncedTextField
					...
					<span className="font-medium">AWS Profile 名称</span>
				</DebouncedTextField>
			) : selectedAuthentication === "apikey" ? (
				<ApiKeyField
					helpText="此密钥仅本地存储，用于此扩展的 API 请求。"
			...
			<p className="mt-1 text-sm text-description">
				{selectedAuthentication === "profile"
					? "使用 ~/.aws/credentials 中的 AWS Profile 凭证。留空配置文件名称则使用默认配置。这些凭证仅本地使用，用于此扩展发出的 API 请求。"
					: "可通过填写上方密钥进行身份验证，或使用默认的 AWS 凭证提供方（如 ~/.aws/credentials 或环境变量）。这些凭证仅本地使用，用于此扩展发出的 API 请求。"}
			</p>
			...
			{showModelOptions && (
				<>
					...
					{isCustomModelSelected && (
						<div>
							<p className="mt-1 text-sm text-description">
									在 Bedrock 中使用应用推理画像时，请选择“自定义”，并在模型 ID 字段中输入应用推理画像的 ARN。
								</p>
							<DebouncedTextField
					...
					{isAdaptiveThinkingModel ? (
						<ReasoningEffortSelector
							...
							description="设为 None 可禁用自适应思考。力度越大响应越详细，但 Token 用量越多。"
							label="自适应思考"
						...
						<ReasoningEffortSelector
							currentMode={currentMode}
							defaultEffort="none"
							description="设为 None 可禁用扩展思考。力度越强思考越深入，但消耗更多 Token。"
							onEffortChange={handleReasoningEffortChange}
						/>
					) : null}
