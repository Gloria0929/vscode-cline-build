...
describe("provider settings edits made before config loads", () => {
	...
	it("OpenAI Compatible: persists an API key typed before the initial read resolves", async () => {
		...
		fireEvent.change(screen.getByPlaceholderText("输入 API 密钥..."), { target: { value: "early-secret" } })
		await flushDebounce()
		...
		// stored key's mask.
		expect(screen.getByPlaceholderText("输入 API 密钥...")).toHaveValue("•".repeat("early-secret".length))
	})
