...
describe("FeatureSettingsSection", () => {
	...
	it("renders 功能提示 toggle in the Editor section", () => {
		const { container } = render(<FeatureSettingsSection renderSectionHeader={() => null} />)

		expect(screen.getByText("功能提示")).toBeTruthy()

		const editorSection = container.querySelector("#optional-features")
		const agentSection = container.querySelector("#agent-features")

		expect(editorSection?.querySelector('[id="功能提示"]')).toBeTruthy()
		expect(agentSection?.querySelector('[id="功能提示"]')).toBeNull()
	})

	it("renders the 自动压缩 Strategy setting in the Agent section", () => {
		const { container } = render(<FeatureSettingsSection renderSectionHeader={() => null} />)

		expect(screen.getByText("自动压缩 Strategy")).toBeTruthy()

		const agentSection = container.querySelector("#agent-features")
		expect(agentSection?.textContent).toContain("Basic")
	})

	it("disables 自动压缩 Strategy when 自动压缩 is off", () => {
		const { container } = render(<FeatureSettingsSection renderSectionHeader={() => null} />)
	...
	it("calls updateSetting with showFeatureTips when toggled", () => {
		...
		const featureTipsSwitch = container.querySelector('[id="功能提示"]')
