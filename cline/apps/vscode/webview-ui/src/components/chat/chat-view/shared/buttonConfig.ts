...
export function getButtonConfig(message: ClineMessage | undefined, _mode: Mode = "act"): ButtonConfig {
	...
	// Special case: command_output should show "运行中继续" button even while streaming
	...
	// Special case: command_output say messages should show "运行中继续" button
	// This allows terminal output to stream while still showing the action button
...
export function buttonsForPhase(
	...
	switch (turnState.phase) {
		case "idle":
			return BUTTON_CONFIGS.default
		case "streaming":
			// A running foreground terminal command offers "运行中继续":
