 ...
 * exercising and debugging Coder's MCP OAuth flow locally, including failure
 * modes such as:
 *
 ...
 *                         for exercising Coder's OAuth state-expiry window).
...
class TestServer {
	...
	private async handleMcp(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
		...
		if (!hasBearer) {
			// This is what triggers Coder's OAuth flow: 401 + WWW-Authenticate
