...
//   MCP tool calls via say/ask="use_mcp_server" with CoderAskUseMcpServer JSON.
...
// - SDK "agent_event" usage → ClineMessage say="api_req_started" with CoderApiReqInfo JSON
...
function sdkToolToClineSayTool(toolName: string, input?: unknown): ClineSayTool {
	...
	switch (toolName) {
		...
		case "replace_in_file": {
			...
			// this the card mislabels a prepend/insert as "Coder wants to create a new file".
 ...
 * "Coder wants to edit this file" row renders only that file's diff (cline#9904).
 *
		...
		case "content_start": {
			switch (event.contentType) {
				...
				case "tool": {
					...
					// with CoderAskUseMcpServer JSON, not generic say="tool".
					const mcpInfo = parseMcpToolName(toolName)
					if (mcpInfo) {
					...
					// All other tools → say="tool" with CoderSayTool JSON
