		...
		return /*html*/ `
			...
			<html lang="en">
				...
				<meta http-equiv="Content-Security-Policy" content="default-src 'none';
					connect-src https://* ws://localhost:* ws://127.0.0.1:* http://localhost:* http://127.0.0.1:* https://*.posthog.com https://*.cline.bot; 
		...
		try {
			...
			if (process.env.IS_DEV) {
				HostProvider.window.showMessage({
					...
					message:
						"Coder: Local webview dev server is not running, HMR will not work. Please run 'bun run dev:webview' before launching the extension to enable HMR. Using bundled assets.",
				})
			}
