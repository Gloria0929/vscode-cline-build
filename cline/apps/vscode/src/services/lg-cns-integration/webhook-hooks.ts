...
export async function writeLgWebhookConfig(webhookUrl: string, webhookToken: string): Promise<void> {
	...
	const clineDir = path.join(documentsPath, "Coder")
...
try {
    ...
    $configPath = Join-Path (Join-Path $documentsPath "Coder") "webhook_config.json"
...
async function readConfig() {
  const configPath = path.join(os.homedir(), "Documents", "Coder", "webhook_config.json")
  try {
