...
export async function activate(context: vscode.ExtensionContext) {
	...
	if (IS_DEV) {
		vscode.commands.executeCommand("setContext", "ai-coder.isDevMode", IS_DEV)
		// Use dynamic import to avoid loading the module in production
		import("./dev/commands/tasks")
			.then((module) => {
				const devTaskCommands = module.registerTaskCommands(webview.controller)
				context.subscriptions.push(...devTaskCommands)
				Logger.log("[Coder Dev] Dev mode activated & dev commands registered")
			})
			.catch((error) => {
				Logger.log(`[Coder Dev] Failed to register dev commands: ${error}`)
			})
	}
		...
		vscode.languages.registerCodeActionsProvider(
			...
			new (class implements vscode.CodeActionProvider {
				...
				provideCodeActions(
					...
					// Explain with Coder (Always available)
					...
					// Improve with Coder (Always available)
					...
					// Fix with Coder (Only if diagnostics exist)
...
\`\`\``
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.Walkthrough, async () => {
			await vscode.commands.executeCommand("workbench.action.openWalkthrough", `${context.extension.id}#AiCoderWalkthrough`)
	...
	Logger.log(`[Coder] extension activated in ${performance.now() - activationStartTime} ms`)
...
function setupHostProvider(context: ExtensionContext) {
	...
	outputChannel.appendLine("[Coder] Setting up VS Code host...")
...
async function openClineSidebarForTaskUri(): Promise<void> {
	...
	Logger.warn("Task URI handling timed out waiting for Coder sidebar visibility")
}
...
async function cleanupLegacyVSCodeStorage(context: ExtensionContext): Promise<void> {
	try {
		...
		// Migrate custom instructions to global Coder rules (one-time cleanup)
